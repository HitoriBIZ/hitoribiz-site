  ...
  return (
    <section className="container py-12">
      ...
      <p className="mt-4 text-neutral-600">
        無料相談・お見積りは下記よりお気軽に。1営業日以内にご返信します。<br/>
        メール：<a className="underline" href="mailto:contact@hitori-biz.com">contact@hitori-biz.com</a>
      </p>

      <div className="mt-8 rounded-2xl border p-6 shadow-soft max-w-2xl">
        {sent ? (
          <div className="text-green-700">送信ありがとうございました。担当よりご連絡いたします。</div>
        ) : (
          <form onSubmit={onSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium">お名前</label>
              <input name="name" required className="w-full border rounded-xl px-3 py-2 mt-1" />
            </div>
            <div>
              <label className="block text-sm font-medium">メール</label>
              <input type="email" name="email" required className="w-full border rounded-xl px-3 py-2 mt-1" />
            </div>
            <div>
              <label className="block text-sm font-medium">ご相談種別</label>
              <select name="topic" className="w-full border rounded-xl px-3 py-2 mt-1">
                <option>アプリ制作</option>
            ...
            <div>
              ...
              <textarea name="message" rows={6} required className="w-full border rounded-xl px-3 py-2 mt-1" />
            </div>
